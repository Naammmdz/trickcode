  ...
  return (
    <div>
      <h2 id="order-heading" data-cy="OrderHeading">
        <Translate contentKey="trickcodeApp.order.home.title">Orders</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="trickcodeApp.order.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/order/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="trickcodeApp.order.home.createLabel">Create new Order</Translate>
          </Link>
        </div>
      </h2>
        ...
        {orderList && orderList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="trickcodeApp.order.id">ID</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                </th>
                ...
                <th>
                  <Translate contentKey="trickcodeApp.order.user">User</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th>
                  <Translate contentKey="trickcodeApp.order.course">Course</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {orderList.map((order, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>
                    <Translate contentKey={`trickcodeApp.OrderStatus.${order.status}`} />
                  </td>
                  ...
                  <td>{order.course ? <Link to={`/course/${order.course.id}`}>{order.course.title}</Link> : ''}</td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="trickcodeApp.order.home.notFound">No Orders found</Translate>
            </div>
          )
        )}
      </div>
