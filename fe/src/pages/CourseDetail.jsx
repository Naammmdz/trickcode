    ...
    return (
      <div className="min-h-screen bg-white dark:bg-[#0a0a0a] flex items-center justify-center">
        <div className="text-center">
          ...
          <Link to="/learn" className="text-primary hover:underline">← Back to Marketplace</Link>
        </div>
      </div>
    );
  }
    ...
    <div className="bg-white dark:bg-[#0a0a0a] text-neutral-900 dark:text-neutral-100 font-sans antialiased overflow-x-hidden selection:bg-primary selection:text-white">
      ...
      <main className="relative z-10 pt-24 pb-12">
        <div className="max-w-7xl mx-auto px-6 mb-8">
          ...
          {/* Breadcrumb */}
          <nav className="flex items-center flex-wrap gap-2 text-[10px] md:text-xs font-sans uppercase tracking-widest text-neutral-500 mb-8">
            ...
            {course.categories && course.categories.length > 0 && (
              <>
                ...
                <Link to={`/learn?category=${course.categories[0].id}`} className="hover:text-neutral-900 dark:hover:text-white transition-colors">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 relative">
            ...
            <div className="lg:col-span-8 space-y-16">
              ...
              <div>
                <div className="flex items-center gap-3 mb-4">
                  ...
                  {rating >= 0 && (
                    <div className="flex items-center gap-1 text-yellow-500 text-xs font-sans">
                      <span className="material-symbols-outlined text-sm" style={{ fontVariationSettings: "'FILL' 1" }}>star</span> {rating.toFixed(1)}
                      <span className="text-neutral-400 dark:text-neutral-500 ml-1">({reviewCount} reviews)</span>
                    </div>
                  )}
                </div>
                ...
                {/* Course Thumbnail */}
                <div className="relative w-full aspect-video bg-gray-50 dark:bg-neutral-900 rounded-lg overflow-hidden shadow-2xl border border-neutral-200 dark:border-neutral-800 flex items-center justify-center">
                  {course.thumbnailUrl ? (
                    <img
                    ...
                    <>
                      ...
                      <span className="material-symbols-outlined text-6xl text-orange-200 dark:text-neutral-700 relative z-10">smart_display</span>
                    </>
                  )}
                </div>
              </div>
              ...
              <section className="border-t border-neutral-200 dark:border-neutral-800 pt-16">
                ...
                <div className="flex flex-col md:flex-row gap-8 items-start">
                  <div className="w-24 h-24 md:w-32 md:h-32 shrink-0 rounded-full bg-neutral-200 dark:bg-neutral-800 flex items-center justify-center border border-neutral-300 dark:border-neutral-700">
                    <span className="material-symbols-outlined text-4xl text-neutral-400">person</span>
                  </div>
                  <div>
                    <h4 className="text-xl font-serif font-medium mb-2">{instructorName}</h4>
                    <p className="text-neutral-600 dark:text-neutral-400 leading-relaxed text-sm md:text-base mb-6">
                      Instructor on TrickCode Platform. Dedicated to teaching and creating high-quality learning content for coding enthusiasts globally.
                    </p>
                ...
                <div className="flex items-end justify-between mb-10">
                  ...
                  <div className="text-right">
                    <div className="flex items-center gap-1 text-yellow-500 mb-1 justify-end">
                      {[1, 2, 3, 4, 5].map((s) => (
                        <span key={s} className={`material-symbols-outlined text-sm ${s <= Math.round(rating) ? 'text-yellow-500' : 'text-neutral-300 dark:text-neutral-600'}`} style={s <= Math.round(rating) ? { fontVariationSettings: "'FILL' 1" } : {}}>
                          star
                        </span>
                      ))}
                    </div>
                    <span className="text-xs font-sans text-neutral-500 uppercase tracking-widest">{rating.toFixed(1)} Rating • {reviewCount} Reviews</span>
                  </div>
                </div>
          ...
          <div className="hidden md:block">
            ...
            {rating >= 0 && (
              <div className="flex items-center gap-2 text-xs text-neutral-500">
                <span className="material-symbols-outlined text-sm text-yellow-500" style={{ fontVariationSettings: "'FILL' 1" }}>star</span> {rating.toFixed(1)} Rating
              </div>
            )}
          </div>
